...
export function readUInt32BE(fileBuffer: Uint8Array, offset: number): number {
        ...
        if (atomType === 'moov') {
            ...
            while (moovOffset < moovEnd) {
                ...
                if (trakType === 'trak') {
                    ...
                    let tkhdOffset: number | null = null;
                    let tkhdSize: number | null = null;
...
export function getMatrix(fileBuffer: Uint8Array, matrixOffset: number): Uint8Array {
    return new Uint8Array(fileBuffer.slice(matrixOffset, matrixOffset + 36));
}

export function modifyTkhdMatrix(fileBuffer: Uint8Array, matrixOffset: number, currentMatrix: Uint8Array, rotation: '90cw' | '90ccw' | '180' = '90cw'): void {

    if (matrixOffset + 36 > fileBuffer.length) return;
    ...
    function findMatchingIndex(matrix: Uint8Array[], target: Uint8Array): number {
        const index = matrix.findIndex(item => areBytesEqual(item, target));
        return index === -1 ? 0 : index;  // Return 0 if no match is found
    }
}

export function calculateMatrixOffset(fileBuffer: Uint8Array, tkhdOffset: number): number {
