/**
 * Interface representing an Edit List Entry
 */
export interface ElstEntry {
    segmentDuration: number;  // Duration of this edit segment (in movie's time scale)
    mediaTime: number;        // Starting time in the media (in media's time scale)
    mediaRateInteger: number; // Media rate (integer part)
    mediaRateFraction: number; // Media rate (fraction part)
  }
  
  /**
   * Interface representing an ELST atom
   */
  export interface ElstAtom {
    offset: number;           // Offset of the ELST atom in the file
    size: number;             // Size of the ELST atom
    version: number;          // Version of the ELST atom
    flags: number;            // Flags of the ELST atom
    entryCount: number;       // Number of entries in the edit list
    entries: ElstEntry[];     // Edit list entries
    trackId?: number;         // Track ID this ELST belongs to (optional)
    timescale: number; // not part of the atom but neded
    mvhdTimescale: number; // not part of the atom but neded
  }
  
  /**
   * Atom box structure in MP4
   */
   ...
   */
  export function findElstAtoms(buffer: Uint8Array): ElstAtom[] {
    const elstAtoms: ElstAtom[] = [];
   ...
   */
  export function findAtom(buffer: Uint8Array, start: number, end: number, type: string): Atom | null {
    let offset = start;
   ...
   */
  export function findAtoms(buffer: Uint8Array, start: number, end: number, type: string): Atom[] {
    const atoms: Atom[] = [];
    let offset = start;
   ...
   */
  export function parseElstAtom(buffer: Uint8Array, offset: number): ElstAtom | null {
    try {
      const size = readUint32(buffer, offset);
  ...
  export function readInt64(buffer: Uint8Array, offset: number): number {
   ...
   export function parseMdhdTimescale(buffer: Uint8Array, startOffset: number): number {
      /**
  ...
  export function modifyElstAtom(buffer: Uint8Array, elstAtom: ElstAtom, startTimeUs: bigint, endTimeUs: bigint) {
    ...
    const scaledSegmentDuration = ( duration / BigInt(1_000_000))* BigInt(elstAtom.mvhdTimescale); 
    
    const mediaTime = (startTimeUs / BigInt(1_000_000)) * BigInt(elstAtom.timescale);
    ...
    function NumberToUint8Array(value: number): Uint8Array {
  ...
  export function parseMvhdTimescale(buffer: Uint8Array, startOffset: number): number {
    /**
     * Parse the MDHD atom to extract the timescale value.
     * 
     * @param data - Binary data containing the MDHD atom
  ...
  console.log(`Found ${elstAtoms.length} ELST atoms`);
  
  elstAtoms.forEach((atom, index) => {
    console.log(`ELST atom #${index + 1} for track ${atom.trackId || 'unknown'}:`);
    console.log(`  Offset: 0x${atom.offset.toString(16)}`);
    console.log(`  Size: ${atom.size} bytes`);
    console.log(`  Version: ${atom.version}`);
    console.log(`  Entry count: ${atom.entryCount}`);
    
    atom.entries.forEach((entry, entryIndex) => {
      console.log(`  Entry #${entryIndex + 1}:`);
      console.log(`    Segment duration: ${entry.segmentDuration}`);
      console.log(`    Media time: ${entry.mediaTime}`);
      console.log(`    Media rate: ${entry.mediaRateInteger}.${entry.mediaRateFraction}`);
    });
  });
  */